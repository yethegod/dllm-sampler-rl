...
Python 3.12, conda env `dllm`. There is no linter config or CI. `tests/` holds self-running `unittest` files (no pytest dependency), all CPU-only: run each with `python tests/<file>.py`; `test_ddp_chunked_backward.py` spins up two gloo ranks and takes ~6 min. `HF_TOKEN` is read from `.env` in the working directory (gitignored).
...
**Single decoding loop, two callers.** `common/generation/generation.py:generate_unified` implements every decoding strategy and is used by *both* training rollouts (`train/trainer.py:_generate_and_score_completions`) and eval (`eval/eval.py:evaluate`). Any change to decoding affects both paths. `remasking` selects the strategy: `policy` (learned per-position), `block_policy` (learned joint block size + threshold, `sampling_mode` `categorical` for training or `categorical-argmax` for greedy eval), `block_schedule` (eval-only control: the same loop driven by a fixed `--block_schedule "b:thres,..."` list instead of a policy, last entry repeating), `block_unmask_policy` (learned block size **and** learned per-position unmasking, no threshold; per-row blocks like `block_policy`, but the policy runs every forward and the record is per step with both heads packed side by side as `[L unmask | K block]` columns, block-decision flag in the last `sampling_masks` column; `block_sampling_mode` picks `categorical` / `categorical-argmax` for the block head while `sampling_mode` stays the per-position mode (`bernoulli` or `dpls` to train; `dpls` records the drawn indices in order, padded with -1, in the same `[L | K]` layout); `block_unmask_window_cond: true` makes the unmask head conditional on the chosen block, p(u | s, b), via a zero-init window embedding and a second trunk pass, and the loop then records a 5-tuple of policy inputs ending in `block_end`), `fastdllm` (confidence threshold `thres`), `low_confidence` / `random` (fixed steps-per-block baselines). When `record_policy_data` is on it also returns the per-timestep policy inputs, samples, and masks needed for the loss.

**Policy** (`common/models/policy.py`): `DiTConfidencePolicy` (main; sees top-p token confidences, 331k params), `DiTBlockSizePolicy` (joint block size + threshold, 332k), `DiTBlockUnmaskPolicy` (block size + per-position unmask: the parent's `output_proj` stays the unmask head and a separate zero-init `boundary_proj` scores block boundaries; shares `boundary_block_logits` with `DiTBlockSizePolicy`; `window_cond=True` adds a zero-init `window_embedding` so the unmask head sees `[block_start, block_end)` while the block head never does), or `DiTHiddenStatePolicy` (ablation; a full `LLaDABlock` at the dLLM's own 4096 width, LLaDA only). `HiddenProjInputMixin` swaps the input stream for the dLLM's last-layer hidden state through a `Linear(dllm_hidden_size -> policy_hidden_dim)`, by overriding `embed_input` alone; mixing it in gives `DiTHiddenProjPolicy` (855k) and `DiTBlockSizeHiddenProjPolicy` (856k), both LLaDA only. Everything from the RoPE DiT stack onward is shared across all of them. Wrapped in `PolicyHFWrapper` so HF Trainer machinery (checkpointing, DDP) treats it as the model. Sampling of the policy logits lives in `common/generation/sampling.py`: `bernoulli` / `dpls` for training, `bernoulli-argmax` for eval of a bernoulli-trained policy, `dpls` (stochastic) or `dpls-greedy` (deterministic `dpls_greedy`, block_unmask only) for a dpls-trained one. `dpls-greedy` is not DPLS's tau->0 limit: that limit commits each position whose utility beats STOP on its own and can drop to one token per step where DPLS commits several.
...
- `block_unmask_policy` with training-mode `bernoulli` truncates rows that pick small blocks. A step can draw nothing, a row's block only advances once fully unmasked, and the only cap is the global `max_steps = L`, so small blocks spend the budget on block-tail stalls and never reach the answer (at p≈0.065: b≤32 ~100% truncated, b≥96 never). The correctness reward then favours large blocks even at alpha=0. The paper's fixed-block `policy` loop does not have this: each block gets `block_length` steps and leftover masks stay mid-text. `dpls` commits ≥1 position per step and cannot truncate.
- Training requires `remasking` in {`policy`, `block_policy`, `block_unmask_policy`} (`train/train.py` asserts the matching `policy_type`/`sampling_mode` per mode); mbpp/humaneval are eval-only datasets.

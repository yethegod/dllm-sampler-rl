...
- The hidden-state policies (`dit_hidden`, `dit_hidden_proj`, `dit_block_size_hidden_proj`) are gated by a literal policy_type tuple in `common/generation/generation.py` (`wants_hidden`, and again in `_compute_policy_logits` for the per-step path and in the `rec_c` buffer + `policy_stream` for the block_policy path). A new hidden policy missing from any of them fails as `NoneType` deep in the decode loop, nowhere near the config. That path also passes LLaDA's `final_hidden_state_only=True` so only the last layer is materialised rather than all 33 (~1.9 GiB/forward at B=16), and it buffers `(B,T,L,d_model)` policy inputs — leave `timestep_batch_size` unset and `compute_loss` casts that whole buffer to fp32 in one go.

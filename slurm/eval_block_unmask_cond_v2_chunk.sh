#!/bin/bash
# One chunk of the v2 block_unmask eval, sized for a 2h ghx4-interactive
...
# eval_llada8b_block_unmask_cond_v2_deltaai.sbatch, split so each piece fits:
#   A  checkpoint-550 greedy (bernoulli-argmax + categorical-argmax), seeds 42,43,44
#   B  checkpoint-550 stochastic block head (categorical), seeds 42,43,44
#   C  checkpoint-50 / 400 / best greedy, seed 42
...
#        bash slurm/eval_block_unmask_cond_v2_chunk.sh A
set -euo pipefail
CHUNK=${1:?chunk A|B|C|D}

REPO=/u/zsun9/dllm-sampler-rl
WORK=/work/hdd/bhta/zsun9
CKPT_DIR=$WORK/checkpoints/llada8b_block_unmask_cond_v2
RESULTS=$WORK/eval_results/llada8b_block_unmask_cond_v2
CONFIG=configs/experiment_configs/llada_8b_instruct_dit_block_unmask_joint_cond_v2.yaml
...
case "$CHUNK" in
  ...
  C) python -m eval.pipeline "$CKPT_DIR" "$CONFIG" --checkpoints first,400,best --seeds 42 \
       --block_sampling_mode categorical-argmax "${common[@]}" ;;
